...
def test_cookie_set_when_absent(client):
    ...
    # В ответе должен быть Set-Cookie с нашим куки
...
def test_cookie_deleted_when_present(client):
    ...
    client.get('/cookies')
    # Теперь делаем запрос с установленным куки
    client.set_cookie(COOKIE_NAME, COOKIE_VALUE)
    response = client.get('/cookies')
    # При удалении Flask устанавливает expires в прошлое (max_age=0)
    set_cookie = response.headers.get('Set-Cookie', '')
    assert COOKIE_NAME in set_cookie
    # Проверяем, что куки удаляется (expires=Thu, 01 Jan 1970 или max-age=0)
...
def test_form_params_get_shows_empty_form(client):
    ...
    html = response.data.decode()
    assert response.status_code == 200
    # Таблица с данными не должна отображаться
    assert 'Полученные данные формы' not in html


# ── 5. Валидация телефона (unit-тесты функции) ────────────────────────────────
...
# ── 6. Страница телефона — интеграционные тесты ───────────────────────────────
