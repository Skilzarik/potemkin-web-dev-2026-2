...
# Пути, которые НЕ логируем (служебные/статические)
...
def log_visit():
    ...
    # пропускаем статику и фавикон
    if any(path.startswith(p) for p in SKIP_LOG_PREFIXES):
        return

    # пропускаем POST'ы и прочие методы, чтобы не плодить дубли;
    # хотя задание говорит "посещения страниц", логичнее писать GET-страницы
