...
class User(UserMixin, db.Model):
    ...
    def can(self, action, target_user=None):
        """Право текущего пользователя на действие.

        action:
            - 'create'           -- создание пользователя
            - 'edit'             -- редактирование пользователя
            - 'delete'           -- удаление пользователя
            - 'show'             -- просмотр карточки пользователя
            - 'view_all_logs'    -- просмотр отчётов по посещениям
        target_user: пользователь, над которым выполняется действие (для edit/show)
        """
